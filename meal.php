...
// Date handling
$current_week_start = isset($_GET['week']) ? new DateTime($_GET['week']) : new DateTime();
$current_week_start->modify('monday this week');
$week_end = clone $current_week_start;
$week_end->modify('+6 days');

// Navigation links
$prev_week = clone $current_week_start;
$prev_week->modify('-1 week');
$next_week = clone $current_week_start;
...
<div class="header">
        ...
        <div class="week-nav">
  <a href="?week=<?= $prev_week->format('Y-m-d') ?>" class="nav-btn">‹ Previous Week</a>
  <div class="current-week">
    <span class="week-label">Current Week</span>
    <span class="week-range"><?= $current_week_start->format('F jS') ?> - <?= $week_end->format('F jS') ?></span>
  </div>
  <a href="?week=<?= $next_week->format('Y-m-d') ?>" class="nav-btn">Next Week ›</a>
</div>
    </div>
    ...
    <div class="user-input-form">
        ...
        <button id="calculate-calories">Calculate</button>
        <p id="daily-calories"></p>
        <p id="weekly-calories"></p>
    </div>

    <!-- Weekly Meal Plan with Drag-and-Drop Feature -->
    <div class="calendar">
        <?php
        $days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
        foreach ($days as $day) {
            echo "<div class='day-content' data-day='{$day}'>";
            echo "<div class='day-header'>{$day}</div>";
            echo "<div class='meal-list'></div>";
            echo "<div class='add-meal'>+ Add Meal</div>";
            echo "<div class='nutrition-info'>Calories: 0 | Protein: 0g | Carbs: 0g</div>";
            echo "</div>";
        }
        ?>
    </div>
    ...
    align-items: center; 
    gap: 20px; /* Adds space between button and canvas */
    margin-top: 20px;
">
    <button id="download-pdf" style="
        padding: 10px 20px; 
        font-size: 16px; 
        background-color: #9c2175; 
        color: white; 
        border: none; 
        border-radius: 5px;
        cursor: pointer;
